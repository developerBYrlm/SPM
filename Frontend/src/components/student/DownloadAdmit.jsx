  ...
  useEffect(() => {
    const getApplication = async () => {
      try {
        ...
        if (res.data.application?.authorityStatus === "Approved") {
          setApproved(true);
        }
      } catch (err) {
        console.error(err);
  ...
  return (
    <div className="admit-main-content">
      <div className="admit-container">
        <h1 className="admit-title">Special Exam Admit Card</h1>
        <p className="admit-subtitle">
          Download your authority approved Special Examination Admit Card.
        </p>
        ...
        {approved ? (
          <>
            ...
            <button type="button" className="download-btn" onClick={downloadAdmit}>
              <i className="fa-solid fa-download"></i> Download Admit Card
            </button>
          </>
        ) : (
          <div className="admit-status pending">
            ⏳ Authority Approval Pending. Admit Card Not Available.
          </div>
        )}
      </div>
    </div>
