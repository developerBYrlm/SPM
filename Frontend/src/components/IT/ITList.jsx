...
import ITButtons from "./ActionButtons/ITActionButtons"; // default import
...
const ITList = () =>  {
  ...
  const columns = [
    ...
    { name: "IT ID", selector: (row) => row.studentId, sortable: true, width: "170px" },
    {
      name: "Image",
      cell: (row) => (
        <img
    ...
    {
      name: "Action",
      cell: (row) => <ITButtons id={row._id} />, width: "290px", center: "true"
    },
  ];
  ...
  useEffect(() => {
    const fetchStudents = async () => {
      setLoading(true);
      try {
        const res = await axios.get("https://spm-1-u37a.onrender.com/api/it", {
  ...
  return (
    <div className="main-content">
      <div className="dashboard-container">
        <h2 className="dashboard-title">Manage IT Team</h2>

        <div className="search-box glass">
          <input
              type="text"
             placeholder="Search by IT ID"
             value={search}
