...
const FApprove = () => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        const apps = res.data.applications.map((app) => {
          ...
          return { ...app, sno: i++, facultyStatus: currentFacultyStatus };
        });
  ...
  // Get faculty profile when page loads
  useEffect(() => {
    fetchFacultyProfile();
  }, []);

  // Get applications after getting faculty profile
  useEffect(() => {
    if (faculty) fetchApplications();
  }, [faculty]);

  // Show approved applications for the current faculty
  ...
  const columns = [
    ...
    {
      name: "Faculty Status",
      selector: (row) => row.facultyStatus,
      sortable: true,
      width: "190px",
    },
    {
